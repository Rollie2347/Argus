...
15. **NOT FIXED — start here next session.** First physical-device test (TestFlight, production build, 2026-07-13) crashed to expo-router's "Unmatched Route" ("this screen doesn't exist") page. User reported this verbally, no console/crash log captured yet — that's the first thing to get. Never reached camera/mic testing, so everything downstream of app launch (known issues #4's audio-session caveat, the actual voice/vision pipeline) is still unverified. **Prime suspect, not yet confirmed:** `mobile/app/_layout.tsx` does an *imperative* redirect in a `useEffect` —
    ```js
    useEffect(() => {
    ...
    ```
    — with an empty `<Stack />` and no `app/index.tsx` route. If the router tries to resolve the initial `/` path before this effect fires (a timing/readiness difference between Expo Go and a compiled standalone build would explain why this wasn't caught in earlier dev testing), there's no matching route to land on, producing exactly this symptom. This is a known expo-router footgun — the documented fix is a declarative `<Redirect />` (or an `app/index.tsx` that renders one) instead of an imperative `router.replace()` racing the navigator's mount, or gating the `<Stack />` render until `getStoredUser()` resolves. **Next session:** (1) get the actual crash/console log from Xcode or `eas build:view` / device logs — don't just patch the hypothesis blind, (2) reproduce if possible via a local EAS development build (`eas build --profile development --platform ios`, since Expo Go's timing may not reproduce it), (3) fix and re-verify via TestFlight, not just a clean bundle/build.
